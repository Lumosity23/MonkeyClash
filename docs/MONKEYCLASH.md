...
- **Intro** : le premier chargement joue l'animation de la bannière du README (`static/images/monkeyclash-banner.svg`, image de `loading.html`). Le SVG s'anime dès qu'il est chargé, donc `loading.ts` ne lui donne sa source qu'une fois la page visible. La page de chargement attend ensuite la fin de l'animation.
