...
- **Intro** : le premier chargement joue l'animation de la bannière du README (« clash » tombe sur « monkeytype » et fait valser « type »). Le mot est un SVG vectoriel aux couleurs du thème, sans le fond de la bannière. Il est généré par `scripts/banner/make_intro.py` dans `loading.html`, et l'animation, qui reprend les timings de la bannière, est dans `loading.scss`. La page de chargement attend la fin de l'animation.
- **Lobby retravaillé** : code du salon en grand en haut à droite (un clic copie le lien d'invitation). La barre de config Monkeytype est en haut à gauche, modifiable par le chef seulement. En haut à droite : les boutons et les joueurs en cartes. Le nom du salon et sa visibilité sont en bas de cette colonne. Le zen est interdit en salon.
...
- `scripts/banner/make_banner.py` et `preview.py` : la bannière animée du README (les commandes sont dans la docstring). `make_intro.py` en tire l'intro vectorielle de la page de chargement.
